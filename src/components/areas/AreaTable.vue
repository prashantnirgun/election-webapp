<template>
  ...
  >
    <template #top="">
      <div class="col-2 q-table__title">Area</div>
      <q-space />
...
export default defineComponent({
  ...
  setup(prop, { emit }) {
    ...
    const onRequest = async (props) => {
      ...
      payload["areas.company_id"] = prop.companyId;
      //payload["$joinRelation"] = "[user,address.area.area]";
      //console.log("payload", payload, props);
      //const returnedData = await api.countries.find(payload);
      //const Areas = useService("areas");
      const returnedData = await Areas.find(payload);
