<template>
  ...
  >
    <template #top>
      <div class="col-2 q-table__title">Ward</div>
      <q-space />
  ...
  setup(prop, { emit }) {
    ...
    const onRequest = async (props) => {
      ...
      const payload = {
        ...
        $limit: rowsPerPage,
        //$joinRelation: "city",
        "$select[]": "manager.volunteer_name as manager",
      };
      ...
      payload["wards.company_id"] = prop.companyId;
      payload["$joinRelation"] = "[manager]";
      //console.log("payload", payload, props);
      //const returnedData = await api.countries.find(payload);
      //const Wards = useService("wards");
      const returnedData = await Wards.find(payload);
