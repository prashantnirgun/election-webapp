<template>
  ...
  >
    <template #top>
      <div class="col-2 q-table__title">{{ prop.routName }}</div>
      <q-space />
...
export default defineComponent({
  ...
  setup(prop, { emit }) {
    ...
    const Volunteers = useService("volunteers");
    ...
    const onRequest = async (props) => {
      ...
      if (prop.routName === "Volunteers") {
        payload["volunteer_type"] = "Volunteer";
      } else {
        payload["volunteer_type"] = "Candidate";
      }
      ...
      //const Volunteers = useService("volunteers");
      const returnedData = await Volunteers.find(payload);
